...
// Auto-delete Student Requests & Complaints after 30 days
cron.schedule('0 0 * * *', async () => {
  try {
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    await Complaint.deleteMany({ createdAt: { $lt: thirtyDaysAgo } });
    
    // Auto-delete Attendance Registry next day (at midnight)
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const attResult = await Attendance.deleteMany({ createdAt: { $lt: today } });
    
    console.log(`[CRON] Daily cleanup: Deleted old complaints and ${attResult.deletedCount} old attendance logs.`);
  } catch (err) {
...
app.post('/api/attendance/sync', async (req, res) => {
  try {
    ...
    // Check for existing record of the SAME type today to prevent duplicates/spam
    ...
    if (!existingLog) {
      // First time for this activity today
      log = new Attendance({
      ...
      // Activity already recorded for today - do not update
      return res.status(200).json({ 
        message: `${typeLabel} already recorded for today at ${existingLog.time}`, 
        log: existingLog 
      });
    }
    ...
    // Calculate against total days in the current month
    const totalDaysInMonth = new Date(istNow.getFullYear(), istNow.getMonth() + 1, 0).getDate();
    const attendancePercentage = Math.min(100, Math.round((distinctDaysThisMonth.length / totalDaysInMonth) * 100));
