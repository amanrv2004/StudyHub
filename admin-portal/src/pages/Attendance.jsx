        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 px-2">
                <div className="flex items-center gap-4 lg:gap-5">
                    <div className="w-10 h-10 lg:w-12 lg:h-12 bg-emerald-600/10 rounded-xl lg:rounded-2xl flex items-center justify-center text-emerald-500 border border-emerald-500/20 shadow-xl shadow-emerald-500/5">
                        <Clock size={20} lg:size={24} strokeWidth={2.5} />
                    </div>
                    <div>
                        <h3 className="text-lg lg:text-2xl font-black text-white uppercase tracking-tighter">Attendance Registry</h3>
                        <p className="text-[9px] lg:text-[10px] text-slate-500 font-black uppercase tracking-widest mt-0.5 lg:mt-1">Real-time activity logs</p>
                    </div>
                </div>

                <div className="flex items-center gap-2 lg:gap-3 w-full lg:w-auto overflow-x-auto no-scrollbar pb-1 lg:pb-0">
                    <div className="px-3 lg:px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-2 shrink-0">
                        <p className="text-[8px] font-black text-emerald-500 uppercase tracking-widest">In:</p>
                        <p className="text-xs lg:text-sm font-black text-white">{checkIns}</p>
                    </div>
                    <div className="px-3 lg:px-4 py-2 bg-rose-500/10 border border-rose-500/20 rounded-xl flex items-center gap-2 shrink-0">
                        <p className="text-[8px] font-black text-rose-500 uppercase tracking-widest">Out:</p>
                        <p className="text-xs lg:text-sm font-black text-white">{checkOuts}</p>
                    </div>
                    <button 
                        onClick={() => { setTypeFilter('Currently In'); setPage(1); }}
                        className={`px-3 lg:px-4 py-2 rounded-xl border transition-all flex items-center gap-2 shrink-0 ${typeFilter === 'Currently In' ? 'bg-blue-500/10 border-blue-500/40 text-blue-400 shadow-lg shadow-blue-500/5' : 'bg-slate-900 border-white/5 text-slate-500'}`}
                    >
                        <p className="text-[8px] font-black uppercase tracking-widest">Active:</p>
                        <p className="text-xs lg:text-sm font-black">{currentlyIn}</p>
                    </button>
                </div>
            </div>

            <Card noPadding className="border-white/5 bg-slate-900/40 backdrop-blur-3xl overflow-hidden">
                <div className="p-4 lg:p-8 border-b border-white/5 bg-slate-950/20 flex flex-col md:flex-row gap-4 lg:gap-6 justify-between items-center">
                    <div className="relative w-full md:w-80 lg:w-96 group">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-emerald-500 transition-colors" size={14} lg:size={16} />
                        <input 
                            type="text" 
                            placeholder="Search Student..." 
                            className="w-full bg-slate-900/50 border border-white/10 rounded-xl py-2.5 lg:py-3 pl-11 lg:pl-12 pr-4 text-[11px] lg:text-xs font-bold text-white outline-none focus:border-emerald-500/50 transition-all placeholder:text-slate-600"
                            value={searchTerm}
                            onChange={(e) => { setSearchName(e.target.value); setPage(1); }}
                        />
                    </div>
                    <div className="w-full md:w-auto overflow-hidden">
                        <div className="flex items-center gap-1.5 bg-slate-900/50 border border-white/10 rounded-xl p-1 overflow-x-auto no-scrollbar shrink-0">
                            {['All', 'Check-In', 'Check-Out', 'Currently In'].map(t => (
                                <button 
                                    key={t}
                                    onClick={() => { setTypeFilter(t); setPage(1); }}
                                    className={`px-3 lg:px-4 py-2 rounded-lg text-[9px] lg:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${typeFilter === t ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'text-slate-600 hover:text-white'}`}
                                >
                                    {t === 'All' ? 'Full View' : t}
                                </button>
                            ))}
